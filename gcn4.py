...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename=BASE_DIR + '/logs/' + now_time + '_gcn4.log')
...
parser.add_argument('--log_msg', help='log_msg', type=str, default="MHA + BCE-loss")
...
class GNNStack(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, lp_path_num, lp_length, lp_dim, lp_w2v_path, ns_length, ns_dim, ns_w2v_path,
        ...
        self.all_mha = torch.nn.MultiheadAttention(input_dim, 8)  # num_heads: 3
    ...
    def forward(self, data):
        ...
        # x_lp_list: (20, cur_nodes_size, lp_length)
        ...
        # 4. MHA
        """
        The reason pytorch requires q, k, and v is that multihead attention can be used either in self-attention OR 
        decoder attention. In self attention, the input vectors are all the same, and transformed using the linear 
        layers you spoke of.

        In decoder attention, the query is based on the current decoder's position, but the key and value are based 
        on the encoder's output. But really you can have all 3 values completely decoupled -- it's just not what model's
        tend to do.
        
        https://www.reddit.com/r/pytorch/comments/c2u6g5/pytorch_multihead_attention_module/
        """
        # x = torch.cat([x_pdt, x_ref, x_def], 1).to(device) #
        # x = x.view([x.shape[0], 1, x.shape[1]])
        # x, attn_output_weights = self.all_mha(x, x, x) # self-attention
        # x = x.view([x.shape[0], x.shape[2]])

        x = torch.stack([x_pdt, x_ref, x_def], 1).to(device)
        x, attn_output_weights = self.all_mha(x, x, x) # self-attention
        x = x.flatten(1)
        ...
        # exit()
        # logger.info("attn_intput: {}".format(x.shape))           # attn_intput: torch.Size([1, 242, 384])
        # logger.info("attn_output: {}".format(attn_output.shape)) # attn_output: torch.Size([1, 242, 384])
...
def train(dataset, task, writer, plot=False, print_grad=False):
    ...
    for epoch in range(100):
        ...
        if total_loss < min_valid_loss:
            ...
            print("Training Loss Decreased: {:.6f} --> {:.6f}. Saved model: {}".format(min_valid_loss, total_loss, best_model_path))
            logger.info("Training Loss Decreased: {:.6f} --> {:.6f}. Saved model: {}".format(min_valid_loss, total_loss, best_model_path))
