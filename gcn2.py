...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    filename=BASE_DIR + '/logs/' + now_time + '_gcn2.log')
...
parser.add_argument('--log_msg', help='log_msg', type=str, default="FC + Concat + BCE-loss")
...
class GNNStack(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, lp_path_num, lp_length, lp_dim, lp_w2v_path, ns_length, ns_dim, ns_w2v_path,
        ...
        fc_dim = 64
        self.fc_ref = nn.Linear(input_dim, fc_dim)
        self.fc_def = nn.Linear(input_dim, fc_dim)
        self.fc_pdt = nn.Linear(input_dim, fc_dim)
        self.all_fc1 = nn.Linear(fc_dim * 3, input_dim)
    ...
    def forward(self, data):
        ...
        x_pdt = F.relu(self.fc_pdt(x_pdt), inplace=True)
        x_ref = F.relu(self.fc_ref(x_ref), inplace=True)
        x_def = F.relu(self.fc_def(x_def), inplace=True)
        x = torch.cat([x_pdt, x_ref, x_def], 1).to(device)
        # logger.info("x after cat: {}".format(x.shape)) # torch.Size([228, 192])
        ...
        # x = (x_ref * x_pdt * x_def).view([cur_nodes_size, self.input_dim * self.input_dim * self.input_dim])
        # x = F.relu(self.bn1(self.all_fc1(x)), inplace=True)
...
def train(dataset, task, writer, plot=False, print_grad=False):
    ...
    for epoch in range(100):
        ...
        if min_valid_loss > total_loss:
            print(f'Training Loss Decreased({min_valid_loss:.6f}--->{total_loss:.6f}) ')
            logger.info(f'Training Loss Decreased({min_valid_loss:.6f}--->{total_loss:.6f}) ')
        ...
        if vali_score['f1'] > best_f1_score:
            ...
            logger.info("saved the best model: " + best_model_path)
